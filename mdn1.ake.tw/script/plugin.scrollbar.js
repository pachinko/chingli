/************ Plugin JQuery ************* */
/* Ralis?par Creamama - Matthieu Lorat */
/* ************ juillet 2010 ************ */ 
...
(function($) {
	$.fn.scrollbar = function(params) {
		// Fusionner les paramtres par dfaut et ceux de l'utilisateur
		params = $.extend( {
			taille_englobe: 500,			//Taille de l'espace visible - /!\ Doit tre un nombre ou 'auto'
			...
			molette: true,					//Dtection du scroll molette - /!\ true ou false
			drag: true,						//Bouton de la scrollbar dplacable ?la souris - /!\ true ou false
			debug: true,					//Afficher la console de debug - /!\ true ou false
			style: 'basic',					//Choix des styles - /!\ Par dfault, il n'y a que le style 'basic'
			position:'droite',				//Position de la scrollBar - /!\ 'gauche' ou 'droite'
			alignement_scrollbar:'haut',	//Alignement de la scrollBar. Utilis?uniquement si elle ?une taille infrieur ?celle de taille_englobe
			orientation: 'vertical',		//Orientation du contenu, 'vertical' ou 'horizontal'
			marge_scroll_contenu: 24,		//Marge entre la scrollBar et le contenu - /!\ Doit tre un nombre
		...
		return this.each(function() {
			...
			//Fonction de calcul du dplacement du bouton
			function deplacement_bouton(info_top_contenu){
				...
				//On vrifie que ca dborde pas en haut
				if(depl_bouton < 0){depl_bouton = 0;}
				//On vrifie que ca dborde pas en bas
			...
			function calcul_hauteur_auto(){
				if(params.taille_englobe == "auto"){
					...
					params.taille_englobe = $(window).height()-0-padBot-padTop;
			...
			function controle_donnee(){
				calcul_hauteur_auto();
				//La taille du contenu doit tre suprieur ?celle de l'espace visible (taille_englobe)
				if(taille_contenu > params.taille_englobe){
					...
					//La taille de la scrollbar doit tre infrieur ou gale ?la taille de taille_englobe
			...
			//Si la hauteur du contenu est suprieur ?la hauteur de l'espace visible
			if(controle_donnee()){
				//Au redimensionnement de la fenetre
				//N'est concern?par cette fonction que les lments en 'auto'
				...
				//On construit une div autour du contenu, mais ?l'intrieur de la div
				...
				switch (params.style) {
					case 'basic':
						...
						$('#scrollbar').css({'width':params.largeur_scrollbar+'px',
											'float':'left',
											'height':params.taille_scrollbar+25+'px',
											'background':'#ebd9b3'		
						});
				...
				if(params.molette){
					$$.mousewheel(function(event, delta) {
						//On rcupre la position du contenu
						var top_contenu = $('#englobe').css('top');
						
						//On enlve le 'px' et on le convertit en entier pour pouvoir le manipuler
						top_contenu = parseInt( top_contenu.substring(0,(top_contenu.length-2)) );
						
						//On rcupre la position du bouton
						...
						if (delta > 0) {							
							...
							//On vrifie que l'on n'a pas atteint le haut du contenu
							if(top_contenu > 0){top_contenu = 0}
							$('#englobe').css({'top':top_contenu+"px"});
						//Si le delta est ngatif, c'est ?dire que l'on "ramne" la molette
						}else if (delta < 0){							
							top_contenu = top_contenu - params.pas;
							//On vrifie que l'on n'a pas atteint le bas du contenu
							if(top_contenu < calcul_contenu_top_max()){top_contenu = calcul_contenu_top_max()}
							$('#englobe').css({'top':top_contenu+"px"});							
						}
						
						//calcul de dplacement du bouton					
